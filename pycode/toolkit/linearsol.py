...
class AugmentedMatrix(object):
    ...
    def __init__(self, m: list, b: list, history='tracked') -> list:
        ...
        try:
            if len(b) != len(m):
                raise ValueError('Error constructing augmented matrix: '
        ...
        self.data = enumerate(
            ...
            ( np.append(m[i], b[i]) for i in range(self.dimension) ), 0
        )
        # Tracked history
...
def triangular_solve(type: str, m: list, b=[]) -> list:
    ...
    if b == []:   # We might have a augmented m; now reject bad matrix:
        ...
        try:
            if len(m) != len(m[0]) - 1:
                raise ValueError('Failed solving triangular system: '
                                 'Dimensions inconsistent!')
            else:
